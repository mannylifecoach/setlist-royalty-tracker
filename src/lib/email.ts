...
<body class="email-bg" style="margin:0;padding:0;background-color:#1a1a1a;color:#ffffff;font-family:'Inter',-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;font-size:13px;letter-spacing:0.5px;text-transform:lowercase;">
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background-color:#1a1a1a;">
    <tr>
      <td align="center" style="padding:40px 20px;">
        <table role="presentation" width="520" cellpadding="0" cellspacing="0" style="max-width:520px;width:100%;background-color:#1a1a1a;">
          <!-- header -->
          <tr>
            <td style="padding-bottom:32px;border-bottom:1px solid #444444;">
              ...
              <br>
              <span style="font-size:13px;font-weight:500;color:#e0e0e0;letter-spacing:0.5px;">setlist royalty tracker</span>
            </td>
          </tr>
          <!-- content -->
          <tr>
            <td style="padding:32px 0;color:#ffffff;">
              ${content}
            </td>
          </tr>
          <!-- footer -->
          <tr>
            <td style="padding-top:32px;border-top:1px solid #444444;">
              <span style="font-size:11px;color:#999999;">you're receiving this because you have an account on setlist royalty tracker.</span>
            </td>
          </tr>
        </table>
      </td>
...
function button(text: string, href: string): string {
  return `<a href="${href}" style="display:inline-block;padding:12px 24px;background-color:#5281eb;color:#ffffff;font-size:13px;font-weight:600;font-family:'Inter',-apple-system,BlinkMacSystemFont,sans-serif;text-decoration:none;text-transform:lowercase;letter-spacing:0.5px;border-radius:4px;">${text}</a>`;
}
...
export async function sendWelcomeEmail(to: string, artistName: string) {
  ...
  const content = `
    ...
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background-color:#252525;border:1px solid #444444;margin-bottom:24px;">
      <tr>
        <td style="padding:16px;border-bottom:1px solid #444444;">
          <span style="font-size:11px;color:#aaaaaa;font-weight:500;">step 1</span>
          <p style="margin:4px 0 0 0;color:#d5d5d5;font-size:13px;">add artists who perform your songs</p>
          <p style="margin:4px 0 0 0;color:#999999;font-size:11px;">we added <strong style="color:#999999;">${artistName}</strong> during setup — add more anytime.</p>
        </td>
      </tr>
      <tr>
        <td style="padding:16px;border-bottom:1px solid #444444;">
          <span style="font-size:11px;color:#aaaaaa;font-weight:500;">step 2</span>
          <p style="margin:4px 0 0 0;color:#d5d5d5;font-size:13px;">register your songs and link them to artists</p>
          <p style="margin:4px 0 0 0;color:#999999;font-size:11px;">the scanner needs to know which songs to look for in setlists.</p>
        </td>
      </tr>
      <tr>
        <td style="padding:16px;border-bottom:1px solid #444444;">
          <span style="font-size:11px;color:#aaaaaa;font-weight:500;">step 3</span>
          <p style="margin:4px 0 0 0;color:#d5d5d5;font-size:13px;">scan for performances</p>
          <p style="margin:4px 0 0 0;color:#999999;font-size:11px;">we search 9.6m+ setlists on setlist.fm to find concerts where your songs were played.</p>
        </td>
      </tr>
      <tr>
        <td style="padding:16px;">
          <span style="font-size:11px;color:#aaaaaa;font-weight:500;">step 4</span>
          <p style="margin:4px 0 0 0;color:#d5d5d5;font-size:13px;">confirm matches and submit to your pro</p>
          <p style="margin:4px 0 0 0;color:#999999;font-size:11px;">use our chrome extension to auto-fill bmi live in seconds (requires google chrome), or export a csv as a backup.</p>
        </td>
      </tr>
    </table>
    ...
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background-color:#252525;border:1px solid #444444;margin-bottom:24px;">
      <tr>
        <td style="padding:16px;border-bottom:1px solid #444444;">
          <span style="font-size:11px;color:#aaaaaa;font-weight:500;">import your sets</span>
          <p style="margin:4px 0 0 0;color:#d5d5d5;font-size:13px;">upload serato dj history csv files</p>
          <p style="margin:4px 0 0 0;color:#999999;font-size:11px;">we match every track against your registered songs — including remixes and edits via musicbrainz.</p>
        </td>
      </tr>
      <tr>
        <td style="padding:16px;">
          <span style="font-size:11px;color:#aaaaaa;font-weight:500;">remix matching</span>
          <p style="margin:4px 0 0 0;color:#d5d5d5;font-size:13px;">automatic work-level matching catches versions others miss</p>
          <p style="margin:4px 0 0 0;color:#999999;font-size:11px;">if a dj plays your track's remix, srt connects it back to your original work using musicbrainz work relationships.</p>
        </td>
      </tr>
    </table>

    <p style="margin:0 0 24px 0;color:#aaaaaa;font-size:12px;">
...
export async function sendNewPerformancesEmail(
  ...
  const songRows = songTitles
    .map(
      (t) =>
        `<tr>
          <td style="padding:8px 12px;border-bottom:1px solid #444444;color:#d5d5d5;font-size:13px;">${t}</td>
          <td style="padding:8px 12px;border-bottom:1px solid #444444;text-align:right;">${statusBadge('discovered', '#5281eb')}</td>
        </tr>`
    )
    .join('');
  ...
  const content = `
    ...
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background-color:#252525;border:1px solid #444444;margin-bottom:24px;">
      <tr>
        <td style="padding:8px 12px;border-bottom:1px solid #444444;color:#aaaaaa;font-size:11px;font-weight:500;">song</td>
        <td style="padding:8px 12px;border-bottom:1px solid #444444;color:#aaaaaa;font-size:11px;font-weight:500;text-align:right;">status</td>
      </tr>
      ${songRows}
    </table>
    ...
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background-color:#252525;border:1px solid #444444;margin-bottom:24px;">
      <tr>
        <td style="padding:16px;text-align:center;">
          ...
          <br>
          <span style="font-size:11px;color:#aaaaaa;">days remaining</span>
        </td>
        <td style="padding:16px;text-align:center;border-left:1px solid #444444;">
          <span style="font-size:32px;font-weight:300;color:#ffffff;letter-spacing:-2px;">${performanceCount}</span>
          <br>
          <span style="font-size:11px;color:#aaaaaa;">performance${plural} at risk</span>
        </td>
      </tr>
    </table>
